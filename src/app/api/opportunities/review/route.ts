...
import { isOpportunityReviewable } from "@/lib/opportunityShortlist";
...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { selectedOpportunityIds?: unknown; reviewedOpportunityIds?: unknown };
    const selectedIds = new Set(parseIds(body.selectedOpportunityIds));
    const reviewedIds = new Set(parseIds(body.reviewedOpportunityIds));
    const data = readArtistData({ opportunityLimit: 1000, opportunityRawContentLimit: 0, applicationLimit: 0 });
    const reviewable = data.opportunities.filter((opportunity) => reviewedIds.has(opportunity.id) && firstReviewStatuses.has(opportunity.status) && isOpportunityReviewable(opportunity));
