...
export default function Studio() {
  ...
  async function setOpportunityDecision(id: number, status: Extract<OpportunityStatus, "selected_by_user" | "not_selected">) {
    setBusy(`opportunity-${id}-${status}`);
    try {
      setData(await api<StudioData>(`/api/opportunities/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ status })
      }));
      setMessage(status === "selected_by_user"
        ? "已选择申请该机会。下一轮自动化会为它准备最终审核包。"
        : "已标记暂不申请。自动化不会为它准备申请包。");
      setError("");
    } catch (err) {
      showError(err);
    } finally {
  ...
  async function setFinalPackageDecision(id: number, decision: "approve_final_submission_package" | "request_revision") {
    ...
    try {
      ...
      setMessage(decision === "approve_final_submission_package"
        ...
        : "已标记需要返工。自动化会把问题写入 internal notes 并重新准备。");
      setError("");
    } catch (err) {
      showError(err);
    } finally {
        ...
        <section className="stack">
          ...
          {tab === "opportunities" && (
            <OpportunitiesPanel
              ...
              prepareCodexContext={prepareCodexContext}
              setOpportunityDecision={setOpportunityDecision}
