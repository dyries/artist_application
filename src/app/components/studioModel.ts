...
export const submissionApprovalModeOptions = [
  ...
  { value: "direct_apply", label: "预授权准备（仍需确认提交）" }
];
...
function hasSourceContent(source: SourceMaterial) {
  return [source.title, source.content, source.analysis, source.fileName, source.filePath].some((value) => value.trim().length > 0);
}
