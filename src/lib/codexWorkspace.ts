...
- Project-internal external API automation uses the provider configured in repository-root \`.env.local\` when available. Users can configure an OpenAI-compatible \`/chat/completions\` gateway or provider-specific APIs such as DeepSeek, OpenAI, Gemini, and Claude.
- Project-internal external API automation may generate reports, verify manually added opportunity links from fetched source text, and draft packages inside the web app.
- When both paths are used, the in-app external API can handle fast drafts and first-pass organization while Codex handles higher-risk or more complex verification, material interpretation, file production, and user-confirmed submission steps.
- Never commit or share API keys.
- Public UI and GitHub-facing docs should describe in-app model capability generically as external API automation rather than making the project appear tied to one provider. Document that users configure external APIs in repository-root \`.env.local\` by copying \`.env.example\` and choosing one provider block.
...
- Every error investigation, bug fix, optimization, or validation result must be recorded in \`docs/fix-log.md\` with date/time, symptom, root cause, changed files, validation commands, and remaining risk. Do not leave fix history only in chat memory or terminal output.
...
- Whenever Codex fixes an error, bug, or optimization, update \`docs/fix-log.md\` in the same change set and include the verification commands that were run.
