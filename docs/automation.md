...
Codex 自动化和网页内外接 API 自动化是两条可自由选择、也可以组合使用的路径。

- 只用 Codex：由 Codex/OpenAI 模型完成实时搜索、网页核验、复杂文件处理、申请包制作和用户确认后的投递步骤；项目不需要外部模型 API key。
- 只用外接 API：由项目读取 `.env.local` 中配置的 API key、base URL 和模型名，在网页内生成报告、核验手动机会链接和生成草稿。
- Codex + 外接 API：网页内 API 负责快速草稿和初步整理，Codex 负责更高风险或更复杂的核验、材料理解、文件制作和投递步骤。
...
Codex automation and in-app external API automation can be used separately or together.
...
- In-app external API only: the app reads the API key, base URL, and model configured in `.env.local` and can generate reports, check manually added opportunity links, and draft packages inside the web app.
- Codex + in-app external API: the in-app API handles fast drafts and first-pass organization, while Codex handles higher-risk or more complex verification, material interpretation, file production, and submission steps.
...
如果只用 Codex 自动化，可以不配置外接 API。

外接 API 配置文件在仓库根目录 `.env.local`。从 `.env.example` 复制后，只启用一个 provider block，填入自己的 key、base URL 和模型名；修改后重启 `npm run dev`。支持多种 API：OpenAI-compatible `/chat/completions` 网关、DeepSeek、OpenAI、Gemini、Claude。不要提交 `.env.local` 或真实 key。

The external API config lives in `.env.local` at the repository root. Copy `.env.example`, enable one provider block, fill in your key, base URL, and model name, then restart `npm run dev`. Supported API types include OpenAI-compatible `/chat/completions` gateways, DeepSeek, OpenAI, Gemini, and Claude. Never commit `.env.local` or real credentials.
...
ARTIST_STUDIO_AI_BASE_URL=https://your-provider.example.com/v1
ARTIST_STUDIO_AI_MODEL=your-model-name
```
...
ARTIST_STUDIO_AI_MODEL=your-deepseek-model
```
